...
local _collectedExitCode = -1
_exit_tests = os.exit
os.exit = function(exitCode)
    print('Wanted to exit with ' .. tostring(exitCode))
    _collectedExitCode = exitCode
end

_test['setup - runtimeConfiguration'] = function()
    _netns.remove_netns('test', true)
    ...
    local _valid, _config = _netns.load_runtime_configuration('test')
    _test.assert(_valid and util.equals(_runtimeConfig, _config, true))
end
...
_test['setup - outboundAddr'] = function()
    ...
    _test.assert(
        _netns.__safe_exec(
            ...
            _runtimeConfigOutboundAddr.vecIp .. '/30',
            '-j SNAT --to-source',
            _options.outboundAddr
        )
    )
end
...
_test['apply iptables (runtime config)'] = function()
    local _options = _netns.get_default_setup_options()
    _options.outboundAddr = '127.0.0.1'
    local _, _runtimeConfig = _netns.load_runtime_configuration('test')
    _test.assert(
        _netns.__safe_exec(
            'iptables -t nat -D POSTROUTING -s',
            _runtimeConfigOutboundAddr.vecIp .. '/30',
